...
3. Не выдумывай характеристики. Если не знаешь — "Уточню у инженера".
4. Никогда не называй закупочные цены или маржу.
5. Отвечай ТОЛЬКО на вопросы о солнечных/ветряных станциях.
6. Не выдумывай характеристики. Если не знаешь — "Уточню у инженера".
7. КРИТИЧЕСКИ ВАЖНО: Если клиент спрашивает про наличие готовых станций на складе, НИКОГДА не отвечай коротко "у нас нет в наличии". Вместо этого используй ТОЛЬКО этот текст:
"Мы проектируем и собираем станции индивидуально под ваш объект, чтобы система работала с максимальной отдачей. У нас есть готовые проверенные конфигурации под разные задачи: от базового комплекта для дачи до мощных систем для бизнеса. 
Мы предлагаем полную свободу: вы можете заказать оборудование с монтажом «под ключ», приобрести только комплект для самостоятельной установки или выбрать гибридный вариант с онлайн-сопровождением нашего инженера. 
Расскажите, какую задачу вы хотите решить (например, полностью отключиться от сети или экономить на счетах)? Напишите, где планируется установка и какая примерная мощность (кВт) вам требуется, и я рассчитаю предварительную стоимость и подготовлю лучшие варианты!"
"""
            ...
            with st.chat_message("assistant"):
                ...
                try:
                    ...
                    if "api_key" in error_msg or "401" in error_msg:
                        ...
                        message_placeholder.error(f"❌ Модель запрещена провайдером (403). Проверьте точное название модели в личном кабинете провайдера.")
